...
[urel_defs]: "Clock offset startTime y = [ true | \<lceil>$y\<acute> =\<^sub>u \<guillemotleft>offset + (if time < startTime then 0 else time - startTime)\<guillemotright>\<rceil>\<^sub>h ]\<^sub>M"
