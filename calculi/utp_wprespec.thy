...
lemma wprespec6a [wp]: "Q \\ (\<And> n\<in>A \<bullet> R(n)) = (\<And> n\<in>A \<bullet> Q \\ R(n))"
  by (rel_auto)

lemma wprespec7 [wp]: "(P \<or> Q) \\ R = ((P \\ R) \<and> (Q \\ R))"
  by (rel_auto)

lemma wprespec7a [wp]: "(\<Sqinter> i\<in>A \<bullet> P(i)) \\ R = (\<And> i\<in>A \<bullet> P(i) \\ R)"
  by (rel_auto)

lemma wprespec8 [wp]: "(P ;; Q) \\ R = P \\ Q \\ R"
