...
definition seq_comp :: "('a \<Rightarrow> 'b::{cmlv,linorder}) \<Rightarrow> 'a bset \<Rightarrow> ('a \<Rightarrow> bool) \<Rightarrow> 'b list" where
...
definition vexpr_seqcomp :: "('a \<Rightarrow> 'b::{cmlv,linorder} cmle * bool cmle) \<Rightarrow> 'a bset cmle \<Rightarrow> 'b list cmle" where
"vexpr_seqcomp FP eA = do { A <- eA
                          ; PA <- cmle_bset_iter A (\<lambda> x. do { c <- snd(FP(x)); if c then LitD {x}\<^sub>b else LitD {}\<^sub>b })
                          ; LA <- cmle_bset_iter (\<Union>\<^sub>b PA) (fst \<circ> FP)
                          ; LitD (blist LA) }"
...
definition vexpr_set_range :: "real cmle \<Rightarrow> real cmle \<Rightarrow> real bset cmle" where
"vexpr_set_range = Op2D' (\<lambda> m n. real `\<^sub>b batLeastAtMost (floor m) (floor n))"

definition vexpr_setcomp :: "('a \<Rightarrow> 'b::{cmlv,linorder} cmle * bool cmle) \<Rightarrow> 'a bset cmle \<Rightarrow> 'b bset cmle" where
...
definition ForallSetD :: "'a bset cmle \<Rightarrow> ('a \<Rightarrow> bool cmle) \<Rightarrow> bool cmle" where
"ForallSetD xs f = MkPExpr (\<lambda> b. (Some (\<forall> x \<in> DestBSet (the (\<lbrakk>xs\<rbrakk>\<^sub>* b)). \<lbrakk>f(x)\<rbrakk>\<^sub>* b = Some True)))"

definition ExistsSetD :: "'a bset cmle \<Rightarrow> ('a \<Rightarrow> bool cmle) \<Rightarrow> bool cmle" where
...
definition vcollect :: "('a \<Rightarrow> bool cmle) \<Rightarrow> 'a bset cmle" where
"vcollect P = MkPExpr (\<lambda> b. BCollect (\<lambda> x. \<lbrakk>P x\<rbrakk>\<^sub>*b))"

definition vcollect_ext :: "('a \<Rightarrow> 'b cmle) \<Rightarrow> ('a \<Rightarrow> bool cmle) \<Rightarrow> 'b bset cmle" where
"vcollect_ext f P = MkPExpr (\<lambda> b. BCollect_ext (\<lambda> x. \<lbrakk>f(x)\<rbrakk>\<^sub>*b) (\<lambda> x. \<lbrakk>P(x)\<rbrakk>\<^sub>*b))"

abbreviation vcollect_ext_ty :: "('a \<Rightarrow> 'b cmle) \<Rightarrow> 'a set \<Rightarrow> ('a \<Rightarrow> bool cmle) \<Rightarrow> 'b bset cmle" where
...
definition vexpr_map_collect :: 
  "('a \<Rightarrow> ('b * 'c) cmle) \<Rightarrow> 'a bset cmle \<Rightarrow> ('b, 'c) bmap cmle" where
"vexpr_map_collect f A = do { A' <- A; g <- cmle_bset_iter A' f; LitD (graph_bmap g)}"
