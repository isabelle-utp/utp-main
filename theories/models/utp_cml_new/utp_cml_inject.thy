...
(* File: utp_cml_inject.thy                                                   *)
(* Author: Simon Foster, University of York (UK)                              *)
(******************************************************************************)

theory utp_cml_inject
imports utp_cml_model
begin
...
text {* To make injecting values into the domain easy, we introduce a type class
  ...
  type. It follows that injecting any value gives a cmlv of the correct vtype.
...
class cmlv =
  fixes Inject  :: "'a \<Rightarrow> cmlv"
  and   Type    :: "'a itself \<Rightarrow> cmlt"
...
definition Project :: "cmlv \<Rightarrow> 'a option" where
...
lemma Project_inj_on:
  "inj_on (the \<circ> (Project :: cmlv \<Rightarrow> 'a option)) (bdcarrier(VTYPE('a)))"
    apply (rule inj_onI)
...
instantiation option :: (cmlv) cmlv
begin

definition Inject_option :: "'a option \<Rightarrow> cmlv" where
"Inject_option x = OptionD VTYPE('a) (x >>= Some \<circ> Inject)"

definition Type_option :: "'a option itself \<Rightarrow> cmlt" where
...
instantiation prod :: (cmlv, cmlv) cmlv
begin

definition Inject_prod :: "'a \<times> 'b \<Rightarrow> cmlv" where
"Inject_prod \<equiv> \<lambda> x. PairD (Inject (fst x)) (Inject (snd x))"
                
definition Type_prod :: "('a \<times> 'b) itself \<Rightarrow> cmlt" where
...
instantiation list :: (cmlv) cmlv
begin

definition Inject_list :: "'a list \<Rightarrow> cmlv" where
"Inject_list xs = ListD VTYPE('a) (map Inject xs)"

definition Type_list :: "'a list itself \<Rightarrow> cmlt" where
...
instance 
  ...
  apply (rule_tac x="map (the \<circ> Project :: cmlv \<Rightarrow> 'a) xs" in exI)
  apply (simp)
  apply (metis Inject_Project_list map_map)
done
end

lemma Type_list: "ListT (VTYPE('a)) = VTYPE(('a::cmlv) list)"
...
instantiation bset :: (cmlv) cmlv
begin

definition Inject_bset :: "'a bset \<Rightarrow> cmlv" where
"Inject_bset xs = SetD VTYPE('a) (bset_image Inject xs)"

definition Type_bset :: "'a bset itself \<Rightarrow> cmlt" where
...
instantiation bmap :: (cmlv, cmlv) cmlv
begin

definition Inject_bmap :: "('a, 'b) bmap \<Rightarrow> cmlv" where
"Inject_bmap f = FuncD VTYPE('a) VTYPE('b) (bset_image (map_prod Inject Inject) (bmap_graph f))" 

definition Type_bmap :: "('a, 'b) bmap itself => cmlt" where
...
lemma bset_image_inverse_lemma:
  ...
  shows "bset_image 
           ...
           (bmap_graph 
              (graph_bmap (bset_image (map_prod (the \<circ> Project :: cmlv \<Rightarrow> 'a) (the \<circ> Project :: cmlv \<Rightarrow> 'b)) F))) = F"
proof -
  from assms have "bfunctional (bset_image (map_prod (the \<circ> Project :: cmlv \<Rightarrow> 'a) (the \<circ> Project :: cmlv \<Rightarrow> 'b)) F)"
    apply (transfer) 
...
instance proof
  ...
  thus "x = y"
  proof -
    have "inj (map_prod (Inject :: 'a \<Rightarrow> cmlv) (Inject :: 'b \<Rightarrow> cmlv))"
  ...
  qed
next
  let ?IJ = "Inject :: ('a,'b) bmap \<Rightarrow> cmlv"
      ...
      proof (safe, simp add:Type_bmap_def)
        ...
        proof -
          have "\<exists>v. Inject (v\<Colon>('a, 'b) bmap) = F" 
            by (metis (no_types) Inject_bmap_def fF fdt ffunc utp_cml_inject.bset_image_inverse_lemma)
          thus "F \<in> range (Inject\<Colon>('a, 'b) bmap \<Rightarrow> cmlv)" by blast
        qed
      qed
    qed
  qed
...
fun InjCML :: "'a::cmlv option \<Rightarrow> cmlv" where
"InjCML None = BotD VTYPE('a)" |
"InjCML (Some x) = Inject x"

definition ProjCML :: "cmlv \<Rightarrow> 'a::cmlv option" where
"ProjCML x = (if (\<D> x) then Project x else None)"

lemma InjCML_inv[simp]: "ProjCML (InjCML x) = x"
  by (case_tac x, auto simp add:ProjCML_def)

lemma InjCML_type[typing]: 
  "InjCML (x :: 'a::cmlv option) :\<^sub>v VTYPE('a)"
  by (case_tac x, auto)

lemma InjCML_defined [defined]: "\<D> x \<Longrightarrow> \<D> (InjCML x)"
  by (case_tac x, simp_all)
  
lemma InjCML_nbot [defined]: "\<D> (InjCML (Some x))"
  by (simp)
