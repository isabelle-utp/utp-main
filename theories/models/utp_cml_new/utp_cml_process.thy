...
definition ForLoopSetD :: "'a bset cmle \<Rightarrow> ('a option \<Rightarrow> cmlp) \<Rightarrow> cmlp" where
"ForLoopSetD xs F = undefined"

definition ParallelD :: 
  "cmlp \<Rightarrow> cmlev bset \<Rightarrow> cmlp \<Rightarrow> cmlp" where
"ParallelD p cs q = ParallelCSP p (LitPE (Abs_USET cs)) q"

definition ReplSeqSetD ::
  "'a bset cmle \<Rightarrow> ('a \<Rightarrow> cmlp) \<Rightarrow> cmlp" where
...
definition ReplAlphaParSetD ::
  "'a bset cmle \<Rightarrow> cmlev set \<Rightarrow> ('a \<Rightarrow> cmlp) \<Rightarrow> cmlp" where
...
"MkChanD n xs \<equiv> AbsChan (bName n, TYPE('a option))"
...
lift_definition Exec3D :: 
  ...
  is "\<lambda> P e f g. {b :: cmlm binding. b \<in> P (e b) (f b) (g b)}" .
...
lift_definition Exec4D :: 
  ...
  is "\<lambda> P e f g h. {b :: cmlm binding. b \<in> P (e b) (f b) (g b) (h b)}" .
...
lift_definition Exec5D :: 
  ...
  is "\<lambda> P e f g h i. {b :: cmlm binding. b \<in> P (e b) (f b) (g b) (h b) (i b)}" .
...
lift_definition Exec6D :: 
  ...
  is "\<lambda> P e f g h i j. {b :: cmlm binding. b \<in> P (e b) (f b) (g b) (h b) (i b) (j b)}" .
...
translations
  "_n_chan_ev c v"   => "CONST insert (CONST PEvent c v) {}"
  "_n_chan_ch c"     => "CONST EventSet {c\<down>}"
  "_n_chan cs"       => "cs"
...
(*  "_n_upred_cml_prefix n p"       == "CONST CommD n p" *)
  ...
  "_cml_proc_abs (_vtype_bind (_vtybind (_vidts x xs) A)) P" 
    => "CONST case_prod (CONST CmlAbsP A (\<lambda> x . (_cml_proc_abs (_vtype_bind (_vtybind xs A)) P)))" 
  ...
  "_cml_proc_abs (_vtype_binds (_vtybind (_vidts x xs) A) bs) P" 
    => "CONST case_prod (CONST CmlAbsP A (\<lambda> x . (_cml_proc_abs (_vtype_binds (_vtybind xs A) bs) P)))"
  "_cml_proc_abs (_vtype_binds (_vtybind (_vidt x) A) bs) P"  
    => "CONST case_prod (CONST CmlAbsP A (\<lambda> x. _cml_proc_abs bs P))"
