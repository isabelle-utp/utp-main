...
overloading
  InjU_vdm  == "InjU  :: 'a::vdmv option \<Rightarrow> vdmm uval" 
  ProjU_vdm == "ProjU :: vdmm uval \<Rightarrow> 'a::vdmv option"
  TypeU_vdm == "TypeU :: 'a::vdmv option itself \<Rightarrow> vdmm utype"
begin

  lift_definition InjU_vdm :: "'a::vdmv option \<Rightarrow> vdmm uval" is "MkVdmv \<circ> InjVDM"
    by (simp add:VALUE_vdmm_def)
    
  lift_definition ProjU_vdm :: "vdmm uval \<Rightarrow> 'a::vdmv option" is "ProjVDM \<circ> DestVdmv" .
  
  lift_definition TypeU_vdm :: "'a::vdmv option itself \<Rightarrow> vdmm utype" is "\<lambda> x. MkVdmt(VTYPE('a))"
    by (simp add:UTYPE_vdmm_def)

end

lemma UTypedef_vdm [typing]: "UTYPEDEF('a option, vdmm)"
  apply (rule UTypedef_intro)
  apply (lifting)
  apply (simp_all add: type_rel_vdmm_def InjU_vdm.rep_eq ProjU_vdm.rep_eq TypeU_vdm.rep_eq typing defined)
  apply (transfer, auto intro:InjVDM_type InjVDM_defined)
  apply (simp add:value_defined_vdmm_alt_def type_rel_vdmm_def)
  apply (transfer, auto simp add:VALUE_vdmm_def)
  apply (metis InjVDM_defined)
  apply (metis DestVdmt.simps InjVDM.simps(2) Inject_type UTYPE_vdmm_def defined_option.simps(1) not_None_eq rangeI)
  apply (simp add:value_defined_vdmm_alt_def type_rel_vdmm_def)
  apply (transfer, auto simp add:VALUE_vdmm_def)
  apply (metis ProjVDM_def Project_def defined_option.simps(2))
  apply (simp add:value_defined_vdmm_alt_def type_rel_vdmm_def)
  apply (transfer, auto simp add:VALUE_vdmm_def)
  apply (metis InjVDM.simps(2) ProjVDM_def Project_Inject Project_def option.sel)
done
...
definition TautDE :: "bool vdme \<Rightarrow> bool" where
...
definition LitD :: "'a \<Rightarrow> 'a vdme" where
...
definition MkVarD :: "string \<Rightarrow> 'a set \<Rightarrow> 'a vdmvar" where
"MkVarD n t \<equiv> MkPlainP n False TYPE('a option) TYPE(vdmm)"
...
definition Op1D :: "('a::vdmv \<rightharpoonup> 'b::vdmv) \<Rightarrow> 'a vdme \<Rightarrow> 'b vdme" where
...
definition Op2D :: "('a::vdmv * 'b::vdmv \<rightharpoonup> 'c::vdmv) 
...
definition Op3D :: "('a::vdmv * 'b::vdmv * 'c::vdmv \<rightharpoonup> 'd::vdmv) 
                   \<Rightarrow> 'a vdme \<Rightarrow> 'b vdme \<Rightarrow> 'c vdme \<Rightarrow> 'd vdme" where
...
abbreviation TokenD :: "'a::vdmv vdme \<Rightarrow> token vdme" where
"TokenD v \<equiv> Op1D' (Abs_token \<circ> Inject) v"

definition ListD :: "'a::vdmv vdme list \<Rightarrow> 'a list vdme" where
"ListD xs = MkPExpr (\<lambda> b. mapM (\<lambda> v. \<lbrakk>v\<rbrakk>\<^sub>* b) xs)"

definition SetD :: "'a::vdmv vdme bset \<Rightarrow> 'a bset vdme" where
"SetD xs = MkPExpr (\<lambda> b. bset_option (bset_image (\<lambda> v. \<lbrakk>v\<rbrakk>\<^sub>* b) xs))"  

abbreviation "NotD     \<equiv> (Op1PE mnot :: bool vdme \<Rightarrow> bool vdme)"
abbreviation "AndD     \<equiv> (Op2PE mconj :: bool vdme \<Rightarrow> bool vdme \<Rightarrow> bool vdme)"
abbreviation "OrD      \<equiv> (Op2PE mdisj :: bool vdme \<Rightarrow> bool vdme \<Rightarrow> bool vdme)"
abbreviation "ImpliesD \<equiv> (Op2PE mimplies :: bool vdme \<Rightarrow> bool vdme \<Rightarrow> bool vdme)"

definition LetD :: "'a vdme \<Rightarrow> 'a set \<Rightarrow> ('a \<Rightarrow> 'b vdme) \<Rightarrow> 'b vdme" where
...
definition Exists1D :: "'a set \<Rightarrow> ('a \<Rightarrow> bool vdme) \<Rightarrow> bool vdme" where
"Exists1D xs f = MkPExpr (\<lambda> b. (Some (\<exists>! x \<in> xs. [\<lbrakk>f(x)\<rbrakk>\<^sub>* b]\<^sub>3)))"

definition IotaD :: "'a set \<Rightarrow> ('a \<Rightarrow> bool vdme) \<Rightarrow> 'a vdme" where 
"IotaD xs f = MkPExpr (\<lambda> b. (if (\<exists>! x \<in> xs. [\<lbrakk>f(x)\<rbrakk>\<^sub>* b]\<^sub>3)
                                then Some (THE x. [\<lbrakk>f(x)\<rbrakk>\<^sub>* b]\<^sub>3)
                                else None))"

definition EpsD :: "'a set \<Rightarrow> ('a \<Rightarrow> bool vdme) \<Rightarrow> 'a vdme" where 
"EpsD xs f = MkPExpr (\<lambda> b. (if (\<exists> x \<in> xs. [\<lbrakk>f(x)\<rbrakk>\<^sub>* b]\<^sub>3)
                                then Some (SOME x. [\<lbrakk>f(x)\<rbrakk>\<^sub>* b]\<^sub>3)
                                else None))"

definition FunD :: "'a set \<Rightarrow> ('a \<Rightarrow> 'b vdme) \<Rightarrow> 'a \<Rightarrow> 'b vdme" where
"FunD A f = (\<lambda> x. if (x \<in> A) then f(x) else \<bottom>\<^sub>v)"

declare FunD_def [evalp]

definition vprod_case ::
  "('a * 'b) set \<Rightarrow> ('a \<Rightarrow> 'b \<Rightarrow> 'c vdme) \<Rightarrow> ('a * 'b) \<Rightarrow> 'c vdme" where
"vprod_case A f = (\<lambda> (x, y). if (x, y) \<in> A then f x y else \<bottom>\<^sub>v)"

declare vprod_case_def [evalp]

definition vfun_case_1 :: 
  "('a * unit) set \<Rightarrow> ('a \<Rightarrow> 'b vdme) \<Rightarrow> ('a * unit) \<Rightarrow> 'b vdme" where
"vfun_case_1 A f = (\<lambda> (x, u). if ((x, u) \<in> A) then f(x) else \<bottom>\<^sub>v)"

definition vfun_case_2 :: 
  "('a * ('b * unit)) set \<Rightarrow> ('a \<Rightarrow> 'b \<Rightarrow> 'c vdme) \<Rightarrow> ('a * ('b * unit)) \<Rightarrow> 'c vdme" where
"vfun_case_2 A f = (\<lambda> (x, (y, u)). if ((x, (y, u)) \<in> A) then f x y else \<bottom>\<^sub>v)"

definition vfun_case_3 :: 
  "('a * ('b * ('c * unit))) set \<Rightarrow> ('a \<Rightarrow> 'b \<Rightarrow> 'c \<Rightarrow> 'd vdme) \<Rightarrow> ('a * ('b * ('c * unit))) \<Rightarrow> 'd vdme" where
...
definition CollectD :: "'a::vdmv vdme \<Rightarrow> bool vdme \<Rightarrow> 'a set" where
...
definition IfThenElseD :: "bool vdme \<Rightarrow> 'a::vdmv vdme \<Rightarrow> 'a vdme \<Rightarrow> 'a vdme" where
...
definition "vexpr_empty       \<equiv> LitD {}\<^sub>b"
definition "vexpr_insert    \<equiv> Op2D' bset_insert"
...
consts
  ApplyD :: "'f::type \<Rightarrow> 'a vdme \<Rightarrow> 'b vdme"

definition ApplyD_fun :: "('a \<Rightarrow> 'b vdme) \<Rightarrow> 'a vdme \<Rightarrow> 'b vdme" where
"ApplyD_fun f x = x >>= f"

definition ApplyD_fun' :: "('a \<Rightarrow> 'b option) \<Rightarrow> 'a vdme \<Rightarrow> 'b vdme" where
...
definition ApplyD_rel :: "('a * 'b) set \<Rightarrow> 'a vdme \<Rightarrow> 'b vdme" where
...
definition plast_bset :: "'a bset \<Rightarrow> 'a bset" where
"plast_bset = id"
...
no_syntax
  ...
  "_n_pexpr_bset_empty"  :: "n_pexpr" ("{}")
  "_n_pexpr_bset"        :: "n_pexprs \<Rightarrow> n_pexpr" ("{_}")
...
syntax
  ...
  "_vexpr_lit"      :: "'a::vdmv \<Rightarrow> n_pexpr" ("(1^_^)")
  "_vexpr_litd"     :: "'a::vdmv \<Rightarrow> n_pexpr" ("(1<<_>>)")
  ...
  "_vexpr_bset"     :: "n_pexprs => n_pexpr"    ("{(_)}")
...
translations
  ...
  "_vexpr_bset (_n_pexprs x xs)" == "CONST vexpr_insert x (_vexpr_bset xs)"
  "_vexpr_bset x"              == "CONST vexpr_insert x CONST vexpr_empty"
...
definition mk_prod :: "'a \<Rightarrow> 'a vdme" where
"mk_prod = LitD"
...
definition VExprTrueT :: "bool vdme \<Rightarrow> vdmp" where
"VExprTrueT e = `@e = true`"

definition VExprDefinedT :: "bool vdme \<Rightarrow> vdmp" ("\<D>\<^sub>v") where
"VExprDefinedT e = `\<not> (@e = \<bottom>)`"

abbreviation VTautT :: "bool vdme \<Rightarrow> vdmp" where
"VTautT e \<equiv> TVL (VExprTrueT e, VExprDefinedT e)"

definition VTautHideT :: "bool vdme \<Rightarrow> vdmp" where
...
lemma EvalE_vdme [evale, evalp, eval]:
  fixes e :: "'a::vdmv vdme"
...
lemma UNREST_PEXPR_SetE [unrest]: 
  "\<lbrakk> \<And> x. x \<in>\<^sub>b xs \<Longrightarrow> vs \<sharp> x \<rbrakk> \<Longrightarrow> vs \<sharp> (SetD xs)"
  apply (simp add:UNREST_PEXPR_def SetD_def)
  apply (clarify)
  apply (auto simp add:fimage.rep_eq bset_option_def bset_image.rep_eq)
  apply (metis (mono_tags) imageI)+
done
...
lemma BotDE_subst [usubst]:
  fixes x :: "('a::vdmv) vdmvar"
  shows "(BotDE :: 'b vdme)[e/\<^sub>*x] = BotDE"
  by (utp_poly_tac)
...
lemma vexpr_insert_defined [defined]:
  ...
  apply (transfer)
  apply (auto intro:defined simp add:bpfun_def vexpr_insert_def)
  apply (rule Op2D_EvalD_defined, auto)
done
...
lemma VTaut_FalseD [simp]:
  ...
  apply (simp del: MkBool_vdmv add:typing defined closure)
done  
*)
