...
(* This law allows integers in VDM (elements of Ints) to
...
(* The following laws are present in the default simplier
   but in reverse. However for VDM we will often need to
...
lemma vin_set_rangeI [intro!]:
  ...
  apply (auto elim!:Ints_elim simp add:evalp batLeastAtMost.rep_eq)
  apply (rule imageI)
  apply (auto)
...
lemma vinset_range [simp]:
  ...
  by (auto simp add:evalp batLeastAtMost.rep_eq)
*)
