...
lift_definition FieldProj :: 
  "('t::tag, 'a::vdmv, 'r) field \<Rightarrow> ('r \<Rightarrow> 'a)" is "fst" ..

declare FieldProj.rep_eq [simp]

lift_definition FieldType ::
  "('t::tag, 'a::vdmv, 'r) field \<Rightarrow> 'a set" is "snd" ..
...
instantiation rec :: (tag, vdmv) vdmv
begin

definition Inject_rec :: "('a, 'b) rec \<Rightarrow> vdmv" where
"Inject_rec r = TagD (tagName (tag :: 'a)) (Inject (Rep_rec r))"

definition Type_rec :: "('a, 'b) rec itself \<Rightarrow> vdmt" where
...
definition UnitField :: "('t::tag, 'a::vdmv, 'r) field \<Rightarrow> ('t, 'a, 'r) tagged" where
"UnitField f = Abs_tagged (FieldType f)"

(* declare UnitField_def [eval,evalp] *)

definition ConsField :: 
  "('t::tag, 'a::vdmv, 'r) field \<Rightarrow> 
...
definition MkTagRec :: "('t::tag, 'r::vdmv, 'r) tagged \<Rightarrow> 'r \<Rightarrow> ('t, 'r) rec" where
"MkTagRec t = Abs_rec"

(* declare MkTagRec_def [eval,evalp] *)

definition MkField :: "('t::tag, 'r) rec set \<Rightarrow> ('r \<Rightarrow> 'a) \<Rightarrow> 'a::vdmv set \<Rightarrow> ('t,'a,'r) field" where
"MkField t f x = Abs_field (f, x)"

definition TermField :: "('t::tag, 'r::vdmv, 's) tagged \<Rightarrow> ('t, 'r) rec set" where
"TermField t \<equiv> Abs_rec ` Rep_tagged t"

abbreviation FinishField :: "('t::tag, 'r::vdmv, 'r) tagged \<Rightarrow> ('t, 'r) rec set" where
...
definition MkRec :: "('t::tag, 'r::vdmv) rec set \<Rightarrow> 'r \<Rightarrow> ('t, 'r) rec vdme" where
