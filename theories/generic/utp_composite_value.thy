...
text {*
  The @{text "SetVal"} constructor might need to know about the type of
  elements the set.
*}
...
begin
definition MkInt_COMPOSITE_VALUE [simp] :
"MkInt_COMPOSITE_VALUE i = BasicVal (MkInt i)"
definition DestInt_COMPOSITE_VALUE [simp] :
"DestInt_COMPOSITE_VALUE v = DestInt (ValOf v)"
definition IsInt_COMPOSITE_VALUE [simp] :
"IsInt_COMPOSITE_VALUE v \<longleftrightarrow> (IsBasicVal v) \<and> IsInt (ValOf v)"
definition MkBool_COMPOSITE_VALUE [simp] :
"MkBool_COMPOSITE_VALUE b = BasicVal (MkBool b)"
definition DestBool_COMPOSITE_VALUE [simp] :
"DestBool_COMPOSITE_VALUE v = DestBool (ValOf v)"
definition IsBool_COMPOSITE_VALUE [simp] :
"IsBool_COMPOSITE_VALUE v \<longleftrightarrow> (IsBasicVal v) \<and> IsBool (ValOf v)"
definition MkStr_COMPOSITE_VALUE [simp] :
"MkStr_COMPOSITE_VALUE s = BasicVal (MkStr s)"
definition DestStr_COMPOSITE_VALUE [simp] :
"DestStr_COMPOSITE_VALUE v = DestStr (ValOf v)"
definition IsStr_COMPOSITE_VALUE [simp] :
"IsStr_COMPOSITE_VALUE v \<longleftrightarrow> (IsBasicVal v) \<and> IsStr (ValOf v)"
definition MkPair_COMPOSITE_VALUE [simp] :
"MkPair_COMPOSITE_VALUE v1_v2 = (uncurry PairVal) v1_v2"
definition DestPair_COMPOSITE_VALUE [simp] :
"DestPair_COMPOSITE_VALUE v = PairOf v"
definition IsPair_COMPOSITE_VALUE [simp] :
"IsPair_COMPOSITE_VALUE v \<longleftrightarrow> IsPairVal v"
definition MkSet_COMPOSITE_VALUE [simp] :
"MkSet_COMPOSITE_VALUE vs = EncSetVal vs"
definition DestSet_COMPOSITE_VALUE [simp] :
"DestSet_COMPOSITE_VALUE v = DecSetOf v"
definition IsSet_COMPOSITE_VALUE [simp] :
"IsSet_COMPOSITE_VALUE v \<longleftrightarrow> IsSetVal v"
instance
apply (intro_classes)
...
"type_rel \<bowtie> (BasicVal v) : (BasicType t) = (type_rel v t)" |
"type_rel \<bowtie> (BasicVal v) : (PairType t1 t2) = False" |
"type_rel \<bowtie> (BasicVal v) : (SetType t) = False" |
"type_rel \<bowtie> (PairVal v1 v2) : (BasicType t) = False" |
"type_rel \<bowtie> (PairVal v1 v2) : (PairType t1 t2) =
 ((type_rel \<bowtie> v1 : t1) \<and> (type_rel \<bowtie> v2 : t2))" |
"type_rel \<bowtie> (PairVal v1 v2) : (SetType t) = False" |
"type_rel \<bowtie> (SetVal vs) : (BasicType t) = False" |
"type_rel \<bowtie> (SetVal vs) : (PairType t1 t2) = False" |
"type_rel \<bowtie> (SetVal vs) : (SetType t) =
 (\<forall> v \<in> DecSet(vs) . (type_rel) \<bowtie> v : t)"
...
"value_ref \<bowtie> (BasicVal v) \<sqsubseteq> (BasicVal v') = (value_ref v v')" |
"value_ref \<bowtie> (BasicVal v) \<sqsubseteq> (PairVal v1' v2') = False" |
"value_ref \<bowtie> (BasicVal v) \<sqsubseteq> (SetVal vs') = False" |
"value_ref \<bowtie> (PairVal v1 v2) \<sqsubseteq> (BasicVal v') = False" |
"value_ref \<bowtie> (PairVal v1 v2) \<sqsubseteq> (PairVal v1' v2') = (v1 = v1' \<and> v2 = v2')" |
"value_ref \<bowtie> (PairVal v1 v2) \<sqsubseteq> (SetVal vs') = False" |
"value_ref \<bowtie> (SetVal vs) \<sqsubseteq> (BasicVal v') = False" |
"value_ref \<bowtie> (SetVal vs) \<sqsubseteq> (PairVal v1' v2') = False" |
"value_ref \<bowtie> (SetVal vs) \<sqsubseteq> (SetVal vs') = (vs = vs')"

subsection {* Locale @{text "COMPOSITE_VALUE"} *}

text {* I am not sure yet whether a locale is really necessary here. *}
