...
definition csp_enable ("\<E>'(_,_,_, _')") where
[upred_defs]: "\<E>(r,s,t,e) = ($tr\<acute> =\<^sub>u $tr ^\<^sub>u \<lceil>t\<rceil>\<^sub>S\<^sub>< \<and> \<lceil>e\<rceil>\<^sub>S\<^sub>< \<notin>\<^sub>u $ref\<acute>) \<triangleleft> s \<triangleright>\<^sub>R \<I>(r)"
    
lemma csp_enable_R1_closed [closure]: "\<E>(r,s,t,e) is R1"
  by (rel_auto)

lemma csp_enable_R2_closed [closure]: "\<E>(r,s,t,e) is R2c"
  by (rel_auto)

lemma csp_enable_unrests [unrest]:
  "\<lbrakk> x \<bowtie> ($tr)\<^sub>v; x \<bowtie> ($tr\<acute>)\<^sub>v; x \<bowtie> ($st)\<^sub>v; x \<bowtie> ($ref\<acute>)\<^sub>v \<rbrakk> \<Longrightarrow> x \<sharp> \<E>(r,s,t,e)"
...
lemma unrest_trea_subst [unrest]:
  ...
  by (simp add: trea_subst_def unrest)
...
definition csp_do :: "_ \<Rightarrow> _ \<Rightarrow> _ \<Rightarrow> ('t::trace, 's) uexpr \<Rightarrow> ('s, 't, 'a) hrel_rsp" ("\<Phi>'(_,_,_,_')") where
[upred_defs]: "\<Phi>(r,s,P,t) = (\<lceil>P\<rceil>\<^sub>S \<and> $tr\<acute> =\<^sub>u $tr + \<lceil>t\<rceil>\<^sub>S\<^sub><) \<triangleleft> s \<triangleright>\<^sub>R \<I>(r)"
  
lemma csp_do_R1_closed [closure]: "\<Phi>(r,s,P,t) is R1"
  apply (rel_auto) using dual_order.trans le_add by blast

lemma unrest_csp_do [unrest]: 
  "\<lbrakk> x \<bowtie> ($tr)\<^sub>v; x \<bowtie> ($tr\<acute>)\<^sub>v; x \<bowtie> ($st)\<^sub>v; x \<bowtie> ($st\<acute>)\<^sub>v \<rbrakk> \<Longrightarrow> x \<sharp> \<Phi>(r,s,P,t)"
...
lemma periR_RR [closure]: "P is NSRD \<Longrightarrow> peri\<^sub>R(P) is RR"
  by (rule RR_intro, simp_all add: unrest closure)

lemma postR_RR [closure]: "P is NSRD \<Longrightarrow> post\<^sub>R(P) is RR"
  by (rule RR_intro, simp_all add: unrest closure)
...
lemma st_pred_CRR [closure]: "[P]\<^sub>S is CRR"
  by (rel_auto)
    
lemma csp_do_assign_enable [rdes]:
  "\<Phi>(r\<^sub>1,s\<^sub>1,\<langle>\<sigma>\<rangle>\<^sub>a,t\<^sub>1) ;; \<E>(r\<^sub>2,s\<^sub>2,t\<^sub>2,e) = 
   \<E>(r\<^sub>1 ^\<^sub>u (\<sigma> \<dagger> r\<^sub>2), s\<^sub>1 \<and> (\<sigma> \<dagger> s\<^sub>2), t\<^sub>1 ^\<^sub>u (\<sigma> \<dagger> t\<^sub>2), \<sigma> \<dagger> e)"
  apply (simp add: csp_do_def csp_enable_def)
  apply (rel_auto)  
    
lemma csp_do_assign [rdes]:
  assumes "P is CRR"
  shows "\<Phi>(r,s,\<langle>\<sigma>\<rangle>\<^sub>a,t) ;; P = (\<lceil>\<sigma>\<rceil>\<^sub>S\<^sub>\<sigma> \<dagger> P)\<lbrakk>t\<rbrakk>\<^sub>t \<triangleleft> s \<triangleright>\<^sub>R \<I>(r)"
proof -
  have "\<Phi>(r,s,\<langle>\<sigma>\<rangle>\<^sub>a,t) ;; CRR(P) = (\<lceil>\<sigma>\<rceil>\<^sub>S\<^sub>\<sigma> \<dagger> CRR(P))\<lbrakk>t\<rbrakk>\<^sub>t \<triangleleft> s \<triangleright>\<^sub>R \<I>(r)"
    apply (rel_auto)
  thus ?thesis
    by (simp add: Healthy_if assms)
qed
    
lemma [usubst]: "\<lceil>\<sigma>\<rceil>\<^sub>S\<^sub>\<sigma> \<dagger> \<lceil>s\<rceil>\<^sub>S\<^sub>\<leftarrow> = \<lceil>\<sigma> \<dagger> s\<rceil>\<^sub>S\<^sub>\<leftarrow>"
  by (rel_auto)
    
lemma [unrest]: "x \<bowtie> ($st)\<^sub>v \<Longrightarrow> x \<sharp> \<lceil>\<sigma>\<rceil>\<^sub>S\<^sub>\<sigma>"
  by (metis lens_comp_left_id st_qual_alpha unrest_subst_alpha_ext)
    
lemma "(\<lceil>\<sigma>\<rceil>\<^sub>S\<^sub>\<sigma> \<dagger> \<E>(r,s,t2,e))\<lbrakk>t1\<rbrakk>\<^sub>t = \<E>(t1 ^\<^sub>u \<sigma> \<dagger> r,\<sigma> \<dagger> s,t1 ^\<^sub>u (\<sigma> \<dagger> t2),\<sigma> \<dagger> e)"
  apply (simp add: csp_enable_def usubst unrest closure)
  apply (rel_auto)
  apply (metis append.assoc less_eq_list_def prefix_concat_minus)
  apply (simp_all add: list_concat_minus_list_concat)
  apply (metis Prefix_Order.same_prefix_prefix append_assoc less_eq_list_def prefix_concat_minus)
  apply (metis Prefix_Order.same_prefix_prefix append.assoc append_prefixD less_eq_list_def prefix_concat_minus)
  apply (simp add: list_append_prefixD)
done
...
lemma csp_do_skip [rdes]:
  assumes "P is CRR"
  shows "\<Phi>(II,t) ;; P = P\<lbrakk>t\<rbrakk>\<^sub>t"
proof -
  have "\<Phi>(II,t) ;; CRR(P) = (CRR P)\<lbrakk>t\<rbrakk>\<^sub>t"
    by (rel_auto)
  thus ?thesis
    by (simp add: Healthy_if assms)
qed
...
lemma wpR_csp_do [wp]:
  fixes P :: "('\<sigma>, '\<phi>) action"
  assumes "P is CRR"
  shows "\<Phi>(\<langle>\<sigma>\<rangle>\<^sub>a,t) wp\<^sub>R P = (\<I>(t) \<Rightarrow>\<^sub>r (\<lceil>\<sigma>\<rceil>\<^sub>S\<^sub>\<sigma> \<dagger> P)\<lbrakk>t\<rbrakk>\<^sub>t)"
proof -
  have "\<Phi>(\<langle>\<sigma>\<rangle>\<^sub>a,t) wp\<^sub>R CRR(P) = (\<I>(t) \<Rightarrow>\<^sub>r (\<lceil>\<sigma>\<rceil>\<^sub>S\<^sub>\<sigma> \<dagger> CRR(P))\<lbrakk>t\<rbrakk>\<^sub>t)"
    by (rel_blast)
  thus ?thesis 
...
lemma wpR_csp_do_skip [wp]:
  ...
  assumes "P is CRR"
  shows "\<Phi>(II,t) wp\<^sub>R P = (\<I>(t) \<Rightarrow>\<^sub>r P\<lbrakk>t\<rbrakk>\<^sub>t)"
proof -
  have "\<Phi>(II,t) wp\<^sub>R P = \<Phi>(\<langle>id\<rangle>\<^sub>a,t) wp\<^sub>R P"
...
lemma periR_DoCSP [rdes]: "peri\<^sub>R(do\<^sub>C(a)) = \<E>(\<langle>\<rangle>,true,\<langle>\<rangle>,a)"
  by (rel_auto, simp_all add: zero_list_def, metis less_eq_list_def strict_prefix_diff_minus) 
...
lemma csp_enable_tr_empty: "\<E>(\<langle>\<rangle>,true,\<langle>\<rangle>,v) = ($tr\<acute> =\<^sub>u $tr \<and> \<lceil>v\<rceil>\<^sub>S\<^sub>< \<notin>\<^sub>u $ref\<acute>)"
  by (rel_auto)
  
lemma periR_PrefixCSP [rdes]:
  assumes "P is NCSP"
  shows "peri\<^sub>R(PrefixCSP a P) = (\<E>(\<langle>\<rangle>,true,\<langle>\<rangle>,a) \<or> (peri\<^sub>R P)\<lbrakk>\<langle>a\<rangle>\<rbrakk>\<^sub>t)"
proof -
  have "peri\<^sub>R(PrefixCSP a P) =  peri\<^sub>R (do\<^sub>C a ;; P)"
    by (simp add: PrefixCSP_def closure assms Healthy_if)
  also have "... = ((\<I>(\<langle>a\<rangle>) \<Rightarrow>\<^sub>r pre\<^sub>R P\<lbrakk>\<langle>a\<rangle>\<rbrakk>\<^sub>t) \<Rightarrow>\<^sub>r $tr\<acute> =\<^sub>u $tr \<and> \<lceil>a\<rceil>\<^sub>S\<^sub>< \<notin>\<^sub>u $ref\<acute> \<or> peri\<^sub>R P\<lbrakk>\<langle>a\<rangle>\<rbrakk>\<^sub>t)"
    by (simp add: assms NSRD_CSP4_intro csp_enable_tr_empty closure rdes unrest ex_unrest usubst rpred wp)
  also have "... = (\<E>(\<langle>\<rangle>,true,\<langle>\<rangle>,a) \<or> ((\<I>(\<langle>a\<rangle>) \<Rightarrow>\<^sub>r pre\<^sub>R P\<lbrakk>\<langle>a\<rangle>\<rbrakk>\<^sub>t) \<Rightarrow>\<^sub>r peri\<^sub>R P\<lbrakk>\<langle>a\<rangle>\<rbrakk>\<^sub>t))"
    by (rel_auto)
  also have "... = (\<E>(\<langle>\<rangle>,true,\<langle>\<rangle>,a) \<or> ((pre\<^sub>R(P) \<Rightarrow>\<^sub>r peri\<^sub>R P)\<lbrakk>\<langle>a\<rangle>\<rbrakk>\<^sub>t))"
    by (rel_auto)
  also have "... = (\<E>(\<langle>\<rangle>,true,\<langle>\<rangle>,a) \<or> (peri\<^sub>R P)\<lbrakk>\<langle>a\<rangle>\<rbrakk>\<^sub>t)"      
...
lemma postR_PrefixCSP [rdes]:
  assumes "P is NCSP"
  shows "post\<^sub>R(PrefixCSP a P) = (\<I>(\<langle>a\<rangle>) \<and> (post\<^sub>R P)\<lbrakk>\<langle>a\<rangle>\<rbrakk>\<^sub>t)"
proof -
  have "post\<^sub>R(PrefixCSP a P) = ((\<I>(\<langle>a\<rangle>) \<Rightarrow>\<^sub>r (pre\<^sub>R P)\<lbrakk>\<langle>a\<rangle>\<rbrakk>\<^sub>t) \<Rightarrow>\<^sub>r (post\<^sub>R P)\<lbrakk>\<langle>a\<rangle>\<rbrakk>\<^sub>t)"
  ...
  also have "... = (\<I>(\<langle>a\<rangle>) \<and> (pre\<^sub>R P \<Rightarrow>\<^sub>r post\<^sub>R P)\<lbrakk>\<langle>a\<rangle>\<rbrakk>\<^sub>t)"
    by (rel_auto)
  also have "... = (\<I>(\<langle>a\<rangle>) \<and> (post\<^sub>R P)\<lbrakk>\<langle>a\<rangle>\<rbrakk>\<^sub>t)"      
...
lemma PrefixCSP_RHS_tri [rdes_def]:
  assumes "P is NCSP"
  shows "PrefixCSP a P = \<^bold>R\<^sub>s ((\<I>(\<langle>a\<rangle>) \<Rightarrow>\<^sub>r pre\<^sub>R P\<lbrakk>\<langle>a\<rangle>\<rbrakk>\<^sub>t) \<turnstile> (\<E>(\<langle>\<rangle>,true,\<langle>\<rangle>, a) \<or> peri\<^sub>R P\<lbrakk>\<langle>a\<rangle>\<rbrakk>\<^sub>t) \<diamondop> post\<^sub>R P\<lbrakk>\<langle>a\<rangle>\<rbrakk>\<^sub>t)"
...
lemma periR_InputCSP [rdes]:
  ...
  shows "peri\<^sub>R(a?(v:A(v)) \<^bold>\<rightarrow> P(v)) =
           ((\<Squnion> x \<bullet> \<E>(\<langle>\<rangle>, A(x), \<langle>\<rangle>, (a\<cdot>\<guillemotleft>x\<guillemotright>)\<^sub>u))) 
              \<triangleleft> $tr\<acute> =\<^sub>u $tr \<triangleright>
            (\<Sqinter> x \<bullet> [A(x)]\<^sub>S \<and> (peri\<^sub>R (P x))\<lbrakk>\<langle>(a\<cdot>\<guillemotleft>x\<guillemotright>)\<^sub>u\<rangle>\<rbrakk>\<^sub>t)" 
...
lemma postR_InputCSP [rdes]:
  ...
  shows "post\<^sub>R(a?(v:A(v)) \<^bold>\<rightarrow> P(v)) =
          (\<Sqinter> x \<bullet> [A x]\<^sub>S \<and> \<I>(\<langle>(a\<cdot>\<guillemotleft>x\<guillemotright>)\<^sub>u\<rangle>) \<and> post\<^sub>R (P x)\<lbrakk>\<langle>(a\<cdot>\<guillemotleft>x\<guillemotright>)\<^sub>u\<rangle>\<rbrakk>\<^sub>t)"
...
lemma GuardedChoiceCSP [rdes_def]:
  ...
  shows "(\<box> x\<in>A \<^bold>\<rightarrow> P(x)) =
             \<^bold>R\<^sub>s ((\<Squnion> x \<in> A \<bullet> \<I>(\<langle>\<guillemotleft>x\<guillemotright>\<rangle>) \<Rightarrow>\<^sub>r pre\<^sub>R (P x)\<lbrakk>\<langle>\<guillemotleft>x\<guillemotright>\<rangle>\<rbrakk>\<^sub>t) \<turnstile>
                 ((\<Squnion> x \<in> A \<bullet> \<E>(true,\<langle>\<rangle>, \<guillemotleft>x\<guillemotright>)) \<triangleleft> $tr\<acute> =\<^sub>u $tr \<triangleright> (\<Sqinter> x \<in> A \<bullet> peri\<^sub>R (P x)\<lbrakk>\<langle>\<guillemotleft>x\<guillemotright>\<rangle>\<rbrakk>\<^sub>t)) \<diamondop>
...
lemma periR_mu_example1 [rdes]:
  "peri\<^sub>R(\<mu> X \<bullet> a \<^bold>\<rightarrow> X) = (\<Sqinter> i \<bullet> \<Phi>(II,\<langle>\<guillemotleft>a\<guillemotright>\<rangle>) \<^bold>^ i ;; \<E>(\<langle>\<rangle>,true,\<langle>\<rangle>, \<guillemotleft>a\<guillemotright>))"
