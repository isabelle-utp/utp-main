...
lemma crf_star_inductl: "\<lbrakk> P is CRF; R is CRR \<rbrakk> \<Longrightarrow> Q \<sqsubseteq> (P ;; Q) \<sqinter> R \<Longrightarrow> Q \<sqsubseteq> P\<^sup>\<star>\<^sup>c ;; R"
  apply (simp add: crf_star_as_rea_star seqr_assoc rpred)
