...
"do\<^sub>I c x P = (
  ...
  (($tr\<acute> - $tr) \<in>\<^sub>u {e : \<guillemotleft>\<delta>\<^sub>u(c)\<guillemotright> | P(e) \<bullet> U([(c\<cdot>\<guillemotleft>e\<guillemotright>)\<^sub>u])} \<and> (c\<cdot>$x\<acute>)\<^sub>u =\<^sub>u last\<^sub>u($tr\<acute>)))"
