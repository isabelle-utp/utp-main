...
proof -
have "ACP1 (R3 P) = `($wait \<and> \<not> ok \<and> ($tr \<le> $tr\<acute>)) \<or> (ok' \<and> $wait \<and> II\<^bsub>REL_VAR - {okay\<down>, okay\<down>\<acute>}\<^esub> \<and> ( ok \<and> ($tr =$tr\<acute>) \<Rightarrow> $wait\<acute>)) \<or> (\<not>$wait \<and> P \<and> ( ok \<and> ($tr =$tr\<acute>) \<Rightarrow> $wait\<acute>))`" by (simp add:ACP1_def R3_def SkipREA_def CondR_def, utp_pred_auto_tac)
also have "... = `($wait \<and> \<not> ok \<and> ($tr \<le> $tr\<acute>)) \<or> (ok' \<and> $wait \<and> $wait\<acute> \<and> II\<^bsub>REL_VAR - {okay\<down>, okay\<down>\<acute>}\<^esub> \<and> ( ok \<and> ($tr =$tr\<acute>) \<Rightarrow> $wait\<acute>)) \<or> (\<not>$wait \<and> P \<and> ( ok \<and> ($tr =$tr\<acute>) \<Rightarrow> $wait\<acute>))`" by (smt AndP_assoc helper1)
also have "... = `($wait \<and> \<not> ok \<and> ($tr \<le> $tr\<acute>)) \<or> (ok' \<and> $wait \<and> $wait\<acute> \<and> II\<^bsub>REL_VAR - {okay\<down>, okay\<down>\<acute>}\<^esub>) \<or> (\<not>$wait \<and> P \<and> ( ok \<and> ($tr =$tr\<acute>) \<Rightarrow> $wait\<acute>))`" by (utp_pred_auto_tac)
also have "... = `($wait \<and> \<not> ok \<and> ($tr \<le> $tr\<acute>)) \<or> (ok' \<and> $wait \<and> II\<^bsub>REL_VAR - {okay\<down>, okay\<down>\<acute>}\<^esub>) \<or> (\<not>$wait \<and> P \<and> ( ok \<and> ($tr =$tr\<acute>) \<Rightarrow> $wait\<acute>))`" by (smt helper1)
also have "... = `R3(P \<and> ( ok \<and> ($tr =$tr\<acute>) \<Rightarrow> $wait\<acute>))`" by(simp add: R3_def CondR_def SkipREA_def, utp_pred_auto_tac)
finally show ?thesis by(simp add:ACP1_def)
qed
...
thus ?thesis by (simp add:R2_def tr_conserved_is_R2 tr_conserved_is_R1 \<delta>_def)
qed
...
lemma \<delta>_is_ACP1 : "\<delta> is ACP1" by(simp add:is_healthy_def \<delta>_def ACP1_R3_commute, simp add:ACP1_def, utp_pred_auto_tac)
...
proof - 
have "\<delta> = `(\<not> ok \<and> $wait \<and> ($tr \<le> $tr\<acute>)) \<or> ((ok' \<and> $wait \<and> II\<^bsub>REL_VAR - {okay\<down>, okay\<down>\<acute>}\<^esub>) \<or> (\<not>$wait \<and> $wait\<acute> \<and>  ($tr = $tr\<acute>)))`" by(simp add:\<delta>_def R3_def SkipREA_def CondR_def,utp_pred_auto_tac)
also have "... = `(\<not> ok \<and> $wait \<and> ($tr \<le> $tr\<acute>)) \<or> $wait\<acute> \<and> ((ok' \<and> $wait \<and> II\<^bsub>REL_VAR - {okay\<down>, okay\<down>\<acute>}\<^esub>) \<or> (\<not>$wait \<and> ($tr = $tr\<acute>)))`"by(simp add:helper1, utp_pred_auto_tac)
finally show ?thesis ..
qed 

lemma doA_is_R1 : "`R1(do\<A>(a))` = `do\<A>(a)`"by(simp add:doA_def \<Phi>_def R_def R1_idempotent)
lemma doA_is_R2 : "`R2(do\<A>(a))` = `do\<A>(a)`"by(simp add:doA_def \<Phi>_def R_def R1_R2_commute R2_idempotent)
lemma doA_is_R3 : "`R3(do\<A>(a))` = `do\<A>(a)`"by(simp add:doA_def \<Phi>_def R_def R2_R3_commute R1_R3_commute R3_idempotent)

lemma doA_closure : "`do\<A>(a)` is R" by(simp add:is_healthy_def R_def doA_is_R3 doA_is_R2 doA_is_R1)

lemma doA_is_ACP1 :"`do\<A>(a)` is ACP1" 
proof -
have "`ACP1(B \<and>( a \<notin> $ref\<acute> \<lhd> $wait\<acute> \<rhd> ($tr^\<langle>a\<rangle> =$tr\<acute>)))` = `(B \<and>( a \<notin> $ref\<acute> \<lhd> $wait\<acute> \<rhd> ($tr^\<langle>a\<rangle> =$tr\<acute>)))`" by(simp add:ACP1_def CondR_def, utp_pred_auto_tac)
moreover have "`ACP1(do\<A>(a))` = `R(ACP1(B \<and>( a \<notin> $ref\<acute> \<lhd> $wait\<acute> \<rhd> ($tr^\<langle>a\<rangle> =$tr\<acute>))))`" by(simp add:doA_def \<Phi>_def R_def ACP1_R1_commute ACP1_R2_commute ACP1_R3_commute)
ultimately show ?thesis by(simp add:is_healthy_def doA_def \<Phi>_def)
qed

lemma B_form : "B = `R1($wait\<acute> \<or> ($tr < $tr\<acute>))`"sorry
...
using assms
proof -
have "R2 (P +\<^bsub>ACP\<^esub> Q) =  `R2(P \<and> Q) \<lhd> R2(\<delta>) \<rhd> R2(P \<or> Q)`" by (simp add:alternative_def R2_distributes_through_conditional CondR_def R2_def R1_def, utp_pred_auto_tac)
also have "... = `(R2(P) \<and> R2(Q)) \<lhd> \<delta> \<rhd> (R2(P) \<or> R2(Q))`" by (simp add: \<delta>_is_R2, simp add: R2_def R1_def R2s_def usubst, utp_pred_auto_tac)
...
using assms
proof -
from assms have "`P \<and> Q` is R3" by(simp add:is_healthy_def R3_def, utp_pred_auto_tac)
moreover from assms have "`P \<or> Q` is R3" by(simp add:is_healthy_def R3_distributes_through_disjunction)
ultimately show ?thesis by(simp add:is_healthy_def alternative_def R3_distributes_through_conditional \<delta>_is_R3) 
qed
...
lemma alternative_is_ACP1 :
  assumes "P is ACP1 \<and> Q is ACP1"
  shows "(P +\<^bsub>ACP\<^esub> Q) is ACP1"
using assms
by(simp add:is_healthy_def ACP1_def alternative_def, utp_pred_auto_tac)
...
lemma L3 : "P +\<^bsub>ACP\<^esub> Q = Q +\<^bsub>ACP\<^esub> P" by(simp add:alternative_def, utp_pred_auto_tac)

lemma L4 : "(P +\<^bsub>ACP\<^esub> Q)+\<^bsub>ACP\<^esub> S = P +\<^bsub>ACP\<^esub> (Q +\<^bsub>ACP\<^esub> S)" by(simp add:alternative_def, utp_pred_auto_tac)
