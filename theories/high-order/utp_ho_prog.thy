...
text {*
  ...
  restriction imposed by the notion of types in HO predicates. However, the
  construction of a formal argument and proof is still on-going work.
*}

subsection {* Program Type Morphisms *}

text {*
  The definition of morphisms requires a leap of faith. That is, we have to
  trust their axiomatisation is sound. I have some doubts this can be shown
  ...
  the cardinality of @{text "HO_VALUE"} (probably some limit cardinal).
*}
...
text {* The following interpretation gives us all the standard typedef laws. *}
...
text {*
  ...
  the @{text VALUE} locale. This paves the way for instantiating the full model
