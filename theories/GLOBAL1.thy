...
(*
  We do not use generic constants for the following two operators anymore. The
  reason is that typing can become subtle with generic constants here, namely
  it is not enough to type the operands in order to type the operator. This can
  lead to unexpected behaviours resulting from instantiation with wrong types.
