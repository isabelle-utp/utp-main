...
"DoBuff \<equiv> (inp?(val) ;; buff := buff @ [val]
           \<box> #buff > 0 && outp!(hd(buff)) ;; buff := tl(buff))"
...
lemma P1: "[true \<turnstile> outps(\<guillemotleft>trace\<guillemotright>) \<le> inps(\<guillemotleft>trace\<guillemotright>) | true]\<^sub>C \<sqsubseteq> Buffer"
proof -
  have "[true \<turnstile> outps(\<guillemotleft>trace\<guillemotright>) \<le> inps(\<guillemotleft>trace\<guillemotright>) | true]\<^sub>C
        \<sqsubseteq>
        Init ;; [true \<turnstile> outps(\<guillemotleft>trace\<guillemotright>) \<le> inps(\<guillemotleft>trace\<guillemotright>) | true]\<^sub>C"
    by (rdes_refine)
  moreover have "Init ;; [true \<turnstile> outps(\<guillemotleft>trace\<guillemotright>) \<le> inps(\<guillemotleft>trace\<guillemotright>) | true]\<^sub>C 
                 \<sqsubseteq>
                 Init ;; [true \<turnstile> outps(\<guillemotleft>trace\<guillemotright>) \<le> (buff @ inps(\<guillemotleft>trace\<guillemotright>)) | true]\<^sub>C"
    by (rdes_refine)
  moreover have "Init ;; [true \<turnstile> outps(\<guillemotleft>trace\<guillemotright>) \<le> (buff @ inps(\<guillemotleft>trace\<guillemotright>)) | true]\<^sub>C
                 \<sqsubseteq>
                 Buffer"
