...
[upred_defs]: "skip\<^sub>m = ($\<^bold>v\<acute> =\<^sub>u $<:\<^bold>v)"
...
lemma U0\<alpha>_indep_right_uvar [simp]: "vwb_lens x \<Longrightarrow> U0\<alpha> \<bowtie> out_var (x ;\<^sub>L mrg_right)"
  by (force intro: plus_pres_lens_indep fst_snd_lens_indep lens_indep_left_comp
            simp add: U0\<alpha>_def out_var_def prod_as_plus)

lemma U1\<alpha>_indep_left_uvar [simp]: "vwb_lens x \<Longrightarrow> U1\<alpha> \<bowtie> out_var (x ;\<^sub>L mrg_left)"
  by (force intro: plus_pres_lens_indep fst_snd_lens_indep lens_indep_left_comp
            simp add: U1\<alpha>_def out_var_def prod_as_plus)
...
lemma U0\<alpha>_comp_in_var [alpha]: "(in_var x) ;\<^sub>L U0\<alpha> = in_var x"
  by (simp add: U0\<alpha>_def alpha_in_var in_var_prod_lens)

lemma U0\<alpha>_comp_out_var [alpha]: "(out_var x) ;\<^sub>L U0\<alpha> = out_var (x ;\<^sub>L mrg_left)"
  by (simp add: U0\<alpha>_def alpha_out_var id_wb_lens out_var_prod_lens)

lemma U1\<alpha>_comp_in_var [alpha]: "(in_var x) ;\<^sub>L U1\<alpha> = in_var x"
  by (simp add: U1\<alpha>_def alpha_in_var in_var_prod_lens)

lemma U1\<alpha>_comp_out_var [alpha]: "(out_var x) ;\<^sub>L U1\<alpha> = out_var (x ;\<^sub>L mrg_right)"
  by (simp add: U1\<alpha>_def alpha_out_var id_wb_lens out_var_prod_lens)
...
[upred_defs]: "ThreeWayMerge M = (($0:\<^bold>v\<acute> =\<^sub>u $0:\<^bold>v \<and> $1:\<^bold>v\<acute> =\<^sub>u $1:0:\<^bold>v \<and> $<:\<^bold>v\<acute> =\<^sub>u $<:\<^bold>v) ;; M ;; U0 \<and> $1:\<^bold>v\<acute> =\<^sub>u $1:1:\<^bold>v \<and> $<:\<^bold>v\<acute> =\<^sub>u $<:\<^bold>v) ;; M"
...
"P \<parallel>\<^sub>s Q \<equiv> (P ;; U0) \<and> (Q ;; U1) \<and> $<\<acute> =\<^sub>u $\<^bold>v"
...
lemma par_by_merge_alt_def: "P \<parallel>\<^bsub>M\<^esub> Q = (\<lceil>P\<rceil>\<^sub>0 \<and> \<lceil>Q\<rceil>\<^sub>1 \<and> $<:\<^bold>v\<acute> =\<^sub>u $\<^bold>v) ;; M"
...
lemma unrest_in_par_by_merge [unrest]:
  "\<lbrakk> $x \<sharp> P; $<:x \<sharp> M; $x \<sharp> Q \<rbrakk> \<Longrightarrow> $x \<sharp> P \<parallel>\<^bsub>M\<^esub> Q"
...
proof -
  have "Q \<parallel>\<^bsub>swap\<^sub>m ;; M\<^esub> P = ((((Q ;; U0) \<and> (P ;; U1) \<and> $<:\<^bold>v\<acute> =\<^sub>u $\<^bold>v) ;; swap\<^sub>m) ;; M)"
    by (simp add: par_by_merge_def seqr_assoc)
  also have "... = (((Q ;; U0 ;; swap\<^sub>m) \<and> (P ;; U1 ;; swap\<^sub>m) \<and> $<:\<^bold>v\<acute> =\<^sub>u $\<^bold>v) ;; M)"
    by (rel_auto)
  also have "... = (((Q ;; U1) \<and> (P ;; U0) \<and> $<:\<^bold>v\<acute> =\<^sub>u $\<^bold>v) ;; M)"
...
proof -
  have "(P \<parallel>\<^bsub>M\<^esub> Q) \<parallel>\<^bsub>M\<^esub> R = ((P ;; U0) \<and> (Q ;; U0 ;; U1) \<and> (R ;; U1 ;; U1) \<and> $<:\<^bold>v\<acute> =\<^sub>u $\<^bold>v) ;; \<^bold>M3(M)"
    by (rel_blast)
  also have "... = ((P ;; U0) \<and> (Q ;; U0 ;; U1) \<and> (R ;; U1 ;; U1) \<and> $<:\<^bold>v\<acute> =\<^sub>u $\<^bold>v) ;; rotate\<^sub>m ;; \<^bold>M3(M)"
    using AssocMerge_def assms(2) by force
  also have "... = ((Q ;; U0) \<and> (R ;; U0 ;; U1) \<and> (P ;; U1 ;; U1) \<and> $<:\<^bold>v\<acute> =\<^sub>u $\<^bold>v) ;; \<^bold>M3(M)"
    by (rel_blast)
...
[upred_defs]: "M[a|b]\<^sub>\<sigma> = ($\<^bold>v\<acute> =\<^sub>u ($<:\<^bold>v \<oplus> $0:\<^bold>v on &a) \<oplus> $1:\<^bold>v on &b)"
