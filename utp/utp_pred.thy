...
lemma unrest_all_diff [unrest]:
  ...
  using assms
  by (pred_auto, simp_all add: lens_indep_comm)
...
lemma USUP_cong_eq:
  ...
  by (simp add: USUP_def, pred_auto, metis)
...
lemma eq_cong_left:
  ...
  using assms
  by (pred_auto, (meson mwb_lens_def vwb_lens_mwb weak_lens_def)+)
...
lemma conj_var_subst:
  ...
  using assms
  by (pred_auto, (metis (full_types) vwb_lens_def wb_lens.get_put)+)
...
lemma eq_split_subst:
  ...
  using assms
  by (pred_auto, metis uvar_assign_exists)
...
lemma ex_equiv:
  ...
  using assms
  by (pred_auto, metis (no_types, lifting) lens.select_convs(2))
...
lemma C7:
  ...
  using assms
  by (pred_auto', simp add: lens_indep_sym)
