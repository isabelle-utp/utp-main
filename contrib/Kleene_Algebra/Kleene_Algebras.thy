...
lemma is_sup_unique: "\<lbrakk> is_sup x A; is_sup y A \<rbrakk> \<Longrightarrow> x = y"
  by (auto simp add: is_sup_def eq_iff)
...
lemma powers_c_elim: "v\<in>(powers_c x y z) \<longleftrightarrow> (\<exists>w. v = x\<cdot>w\<cdot>z \<and> (\<exists>i. w = power y i))"
  by (auto simp add: powers_c_def)

lemma powers_to_powers_c: "powers x = powers_c 1 x 1"
  by (auto simp add: powers_c_elim powers_def)
...
lemma powers_sucl: "powers_c x x 1 = {y. (\<exists>i. y = power x (Suc i))}"
  using powers_c_elim power_in_powers_c by (auto, blast)

lemma powers_sucr: "powers_c 1 x x = {y. (\<exists>i. y = power x (Suc i))}"
  using powers_c_elim power_commutes power_in_powers_c
  by (auto, blast, metis)
...
proof -
  ...
  proof -
    have "\<exists>i. 1 = x ^ i"
      by (rule_tac x="0 :: nat" in exI, simp)
    thus ?thesis
      by (auto, metis (mono_tags) nat.exhaust power.simps)+
  qed
...
lemma power_inductl_var: "x\<cdot>y \<le> y \<Longrightarrow> (x^n)\<cdot>y \<le> y"
proof (induct n)
  case 0 thus ?case by auto
next
  case (Suc n) 
  hence "x \<cdot> x ^ n \<cdot> y \<le> y"
    using local.dual_order.trans local.mult_isol mult_assoc by (presburger)
  thus ?case
    by simp
qed

lemma power_inductr_var: "y\<cdot>x \<le> y \<Longrightarrow> y\<cdot>(x^n) \<le> y"
proof (induct n)
  case 0 thus ?case by auto
next
  case (Suc n)  
  hence "y \<cdot> (x \<cdot> x ^ n) \<le> y"
    by (metis (no_types, lifting) local.dual_order.trans local.mult_isor mult_assoc)
  thus ?case
    by simp
qed
...
lemma sup_un: "is_sup w A \<Longrightarrow> is_sup (x+w) ({x}\<union>A)"
  by (auto simp add: is_sup_def add_lub)

end

class star_continuous_ka = dioid_one_zero + star_op +
  assumes ex_star: "\<forall>x y z. \<exists>w. is_sup w (powers_c x y z)"
  and star_def:"x\<cdot>(y\<^sup>\<star>)\<cdot>z = (THE w. is_sup w (powers_c x y z))"
begin

lemma prod_star_unique: "is_sup w (powers_c x y z) \<Longrightarrow> x \<cdot> y\<^sup>\<star> \<cdot> z = w"
  by (metis star_def is_sup_unique the_equality)

lemma star_unique: "is_sup w (powers x) \<Longrightarrow> x\<^sup>\<star> = w"
  by (metis mult_onel mult_oner powers_to_powers_c prod_star_unique)

lemma prod_star_is_sup: "is_sup (x \<cdot> y\<^sup>\<star> \<cdot> z) (powers_c x y z)"
  by (metis ex_star prod_star_unique)

lemma star_lub: "(\<forall>v\<in>(powers_c x y z). v \<le> w) \<longleftrightarrow> x \<cdot> y\<^sup>\<star> \<cdot> z \<le> w"
  by (metis (full_types) is_sup_def prod_star_is_sup)

lemma cstar_unfoldl: "1 + x \<cdot> x\<^sup>\<star> = x\<^sup>\<star>"
proof -
...
lemma cstar_unfoldr: "1 + x\<^sup>\<star> \<cdot> x = x\<^sup>\<star>"
  by (metis mult_onel mult_oner powers_suc prod_star_is_sup prod_star_unique cstar_unfoldl)

lemma cstar_inductl: "x\<cdot>y \<le> y \<longrightarrow> x\<^sup>\<star> \<cdot> y \<le> y"
  by (metis power_inductl_var mult_onel powers_c_elim star_lub)

lemma cstar_inductr: "y\<cdot>x \<le> y \<longrightarrow> y \<cdot> x\<^sup>\<star> \<le> y"
...
proof
  fix x
  show "1 + x \<cdot> x\<^sup>\<star> \<le> x\<^sup>\<star>"
    by (simp add: cstar_unfoldl)
next
  fix x y z
  show "z + x \<cdot> y \<le> y \<longrightarrow> x\<^sup>\<star> \<cdot> z \<le> y"
    using cstar_inductl local.add_lub local.dual_order.trans local.mult_isol by blast
next
  fix x y z
  show "z + y \<cdot> x \<le> y \<longrightarrow> z \<cdot> x\<^sup>\<star> \<le> y"
    using cstar_inductr local.add_lub local.dual_order.trans local.mult_isor by blast
qed
end
