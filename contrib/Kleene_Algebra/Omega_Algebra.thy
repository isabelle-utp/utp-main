...
definition top_omega ("\<top>\<^sub>\<omega>")
  where "\<top>\<^sub>\<omega> = 1\<^sup>\<omega>"

lemma star_omega_3 [simp]: "(x\<^sup>\<star>)\<^sup>\<omega> = \<top>\<^sub>\<omega>"
proof -
  have "1 \<le> x\<^sup>\<star>"
    by (fact star_ref)
  hence "\<top>\<^sub>\<omega> \<le> (x\<^sup>\<star>)\<^sup>\<omega>"
    by (metis omega_iso top_omega_def)
  thus ?thesis
    by (metis eq_iff max_element top_omega_def)
qed
...
lemma omega_top [simp]: "x\<^sup>\<omega> \<cdot> \<top>\<^sub>\<omega> = x\<^sup>\<omega>"
  by (metis max_element omega_sup_id top_omega_def)

lemma supid_omega: "1 \<le> x \<longrightarrow> x\<^sup>\<omega> = \<top>\<^sub>\<omega>"
  by (metis eq_iff max_element omega_iso top_omega_def)

lemma "x\<^sup>\<omega> = \<top>\<^sub>\<omega> \<longrightarrow> 1 \<le> x"
  nitpick [expect=genuine] -- "4-element counterexample"
oops
...
proof (rule antisym)
  have "(x\<^sup>\<omega>)\<^sup>\<star> = 1 + x\<^sup>\<omega> \<cdot> (x\<^sup>\<omega>)\<^sup>\<star>"
    by simp
  also have "... \<le> 1 + x\<^sup>\<omega> \<cdot> \<top>\<^sub>\<omega>"
