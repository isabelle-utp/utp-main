...
subsection \<open>Dynamic representation\<close>
...
locale inca =
  ...
  for
    F_empty and
    F_get :: "'fenv \<Rightarrow> 'fun \<Rightarrow> ('dyn, 'var, 'fun, 'op, 'opinl) instr fundef option" and
...
inductive step ::
  "('fenv, 'henv, ('fun, 'dyn) frame) state \<Rightarrow> ('fenv, 'henv, ('fun, 'dyn) frame) state \<Rightarrow> bool" (infix "\<rightarrow>" 55) where
...
lemma step_deterministic:
  "s1 \<rightarrow> s2 \<Longrightarrow> s1 \<rightarrow> s3 \<Longrightarrow> s2 = s3"
...
sublocale inca_sem: semantics step final
